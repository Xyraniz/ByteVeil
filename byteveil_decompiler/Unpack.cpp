...
std::string inspect(const std::string& data){
    ...
    if (family == "MoonSec V3") {
        std::string result = MoonSec::inspect(data);
        if (result.find("\"status\":\"serialized-bytecode-extracted\"") != std::string::npos)
            return result;
        // Preserve the generic literal-loadstring result below when a visible
        // MoonSec marker belongs to an unrelated text file.  The native
        // adapter's honest failure reason remains available through
        // `--format moonsec`.
    }
